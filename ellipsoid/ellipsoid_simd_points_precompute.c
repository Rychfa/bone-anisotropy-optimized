...
#define RESIDUAL_ACC(k,i) \
	...
	VAR(p0p0,k) = _mm256_load_pd(&p0p0[i]);\
	VAR(p0p1,k) = _mm256_load_pd(&p0p1[i]);\
	VAR(p0p2,k) = _mm256_load_pd(&p0p2[i]);\
	VAR(p1p1,k) = _mm256_load_pd(&p1p1[i]);\
	VAR(p1p2,k) = _mm256_load_pd(&p1p2[i]);\
	VAR(p2p2,k) = _mm256_load_pd(&p2p2[i]);\
	\
...
static double _cost(const double *p0p0, 
	const double *p0p1, 
	const double *p0p2, 
	const double *p1p1, 
	const double *p1p2, 
...
void fit_ellipsoid_simd_points_precompute(const double (*p)[3], int n, double (*Q)[3][3])
{
	...
	/* shuffle variables and precompute the outer product once and store
	 * note this modifies the flop count!
	 */
	static double p0p0[MAX_ARRAY_DIM] = {0};
	static double p0p1[MAX_ARRAY_DIM] = {0};
	static double p0p2[MAX_ARRAY_DIM] = {0};
	static double p1p1[MAX_ARRAY_DIM] = {0};
	static double p1p2[MAX_ARRAY_DIM] = {0};
	static double p2p2[MAX_ARRAY_DIM] = {0};
	...
	for (i=0; i<n-4; i+=4) {
		...
		__m256d p0p0_0, p0p1_0, p0p2_0, p1p1_0, p1p2_0, p2p2_0;
		p0p0_0 = _mm256_mul_pd(p0_0, p0_0);
		p0p1_0 = _mm256_mul_pd(p0_0, p1_0);
		p0p2_0 = _mm256_mul_pd(p0_0, p2_0);
		p1p1_0 = _mm256_mul_pd(p1_0, p1_0);
		p1p2_0 = _mm256_mul_pd(p1_0, p2_0);
		p2p2_0 = _mm256_mul_pd(p2_0, p2_0);

		_mm256_store_pd(&p0p0[i], p0p0_0);
		_mm256_store_pd(&p0p1[i], p0p1_0);
		_mm256_store_pd(&p0p2[i], p0p2_0);
		_mm256_store_pd(&p1p1[i], p1p1_0);
		_mm256_store_pd(&p1p2[i], p1p2_0);
		_mm256_store_pd(&p2p2[i], p2p2_0);
	}
	for (;i<n; i++) {
		p0p0[i] = p[i][0]*p[i][0];
		p0p1[i] = p[i][0]*p[i][1];
	...
	while(1) {
		...
		while (_cost(p0p0, p0p1, p0p2, p1p1, p1p2, p2p2, n, Qnext00, Qnext01, Qnext02, Qnext11, Qnext12, Qnext22) > 
