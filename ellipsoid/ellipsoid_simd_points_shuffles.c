...
#define RESIDUAL_ACC(k,i) \
	...
	VAR(p0,k) = _mm256_i32gather_pd(&p[i][0], vINDEX, sizeof(double));\
	VAR(p1,k) = _mm256_i32gather_pd(&p[i][1], vINDEX, sizeof(double));\
	VAR(p2,k) = _mm256_i32gather_pd(&p[i][2], vINDEX, sizeof(double));\
	\
 ...
 */
#if 1
static double _cost(const double (*p)[3], int n,
...
void fit_ellipsoid_simd_points_shuffles(const double (*p)[3], int n, double (*Q)[3][3])
{
	...
	while(1) {
		...
		while (_cost(p, n, Qnext00, Qnext01, Qnext02, Qnext11, Qnext12, Qnext22) > 
